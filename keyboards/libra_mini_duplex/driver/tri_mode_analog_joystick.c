...
int16_t calibrate(int16_t position, uint16_t origin, uint8_t deadZone) {
    int8_t  direction;
    ...
    float   coordinate    = (float)distanceFromOrigin / range;
    int16_t percent = (int16_t)(coordinate * 128);
    if (percent < 0) {
        return 0;
    } else 
    if (percent > 127) {
...
void axisCoordinate(pin_t pinX, pin_t pinY, uint16_t originX, uint16_t originY, int16_t *percentX, int16_t *percentY) {
    int16_t positionX = analogReadPin(pinX);
    int16_t positionY = analogReadPin(pinY);
    dprintf("%d %d\n", positionX, positionY );

    *percentX = calibrate(positionX, originX, 10);
    *percentY = calibrate(positionY, originY, 30);
...
int map(int16_t s) {//, int a1, int a2, int b1, int b2) {
...
void axisToKeycode(int16_t percent, uint8_t rowH, uint8_t rowL, uint8_t colH, uint8_t colL) {
...
void joyToDigitalKeycode(int16_t xpercent, int16_t ypercent) {
    
    joy_digi_state = 0;
    int16_t xpos = xpercent;
    if (xpos < -20) {
        //arrow_state right
        joy_digi_state = 0x8;
    }
...
report_analog_joystick_t analog_joystick_read(void) {
    ...
    if (timer_elapsed(lastCursor) > ANALOG_JOYSTICK_READ_INTERVAL) {
        ...
        int16_t x, y;
        axisCoordinate(ANALOG_JOYSTICK_X_AXIS_PIN, ANALOG_JOYSTICK_Y_AXIS_PIN, xOrigin, yOrigin, &x, &y);
        ...
        if ( ths_lyr_kmap_to_kcode(8,0) == JS_PD ) {
            ...
            joystick_set_axis(0,(x*8));;
            joystick_set_axis(1,(y*8));;
#endif
