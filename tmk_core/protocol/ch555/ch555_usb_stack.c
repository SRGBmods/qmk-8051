...
#include "hal.h" //for clock config
#include "usb_device.h" //for USB device & Endpoint init, location for EndPoint buffers
...
host_driver_t  ch555_usb_stack_driver = {keyboard_leds, send_keyboard, send_mouse, send_extra};
...
void send_report(uint8_t endpoint, void *report, size_t size) {
	...
	memcpy( pUSB_BUF_DEV0 + (UX_EP1_ADDR<<(endpoint-1)), (uint8_t *)report, size );
    uint8_t volatile __xdata *D0_EPxRES = XSFR_USBD0_BASE + endpoint;
    uint8_t volatile __xdata *D0_EPxT_L = D0_EPxRES + 8;

	while( ( *D0_EPxRES & MASK_UEP_X_RES ) == UEP_X_RES_ACK );                  
	
	*D0_EPxRES ^= bUEP_X_TOG;	
	*D0_EPxT_L = size;                                       
...
static void send_extra(report_extra_t *report) {
    ...
    //send_report(5, report, sizeof(report_extra_t));
#endif
}
...
//    //D0_EP_MOD |= bUX_EP2O_EN;                 //EP1 IN & EP2 OUT
//    HID_writeBusyFlag = 0;                    // reset write busy flag
//}
//
//// Endpoint 1 IN handler (HID report transfer to host completed)
//void HID_EP1_IN(void) {
//  D0_EP1T_L = 0;                                           // no data to send anymore
//  D0_EP1RES = D0_EP1RES & ~MASK_UEP_X_RES | UEP_X_RES_NAK;  // default NAK
...
#else
    //USB_USBTask(); //TODO call USB_Device_ProcessControlRequest?? why?
#endif
}
    ...
    if (USB_DeviceState == DEVICE_STATE_Suspended) {
        ...
        while (USB_DeviceState == DEVICE_STATE_Suspended) {
            ...
            if (USB_Device_RemoteWakeupEnabled && suspend_wakeup_condition()) {
                //USB_Device_SendRemoteWakeup(); TODO implement this
                clear_keyboard();
...
void protocol_post_task(void) {
	...
	if( MCU_Sleep_Operate )
	{
		...
		SAFE_MOD = 0x55;
		SAFE_MOD = 0xAA;
		WAKE_CTRL |= bWAK_BY_USB;       									/* Wake up when there is signal from Usb */			
//		POWER_CFG |= bPWR_DN_MODE;		
		SAFE_MOD = 0x00;
		CH55X_Sleep_Deal( );  												/* Execute microcontroller sleep operation */
		EA = 1; 	
	}
}
